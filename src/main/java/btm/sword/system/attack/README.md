...
| `GeneratedAttackProfile` | Implements `AttackProfile` with runtime-computed `ControlVectors`. Used when the sweep geometry is built dynamically at attack time (e.g., the heavy sweep targeting a specific entity). |
...
| Class | Description |
|-------|-------------|
| `AttackProfile` | Interface: `controlVectors()`, `knockbackFunction()`, `normalVector()`. Implemented by both `AttackType` and `GeneratedAttackProfile`. |
| `AttackType` | Enum of named attack curves. Each entry holds a `ControlVectors` (four canonical-space vectors) and a `Function<Attack, Vector>` for knockback direction. |
...
3. `startAttack()` captures the attacker's current `Basis` (local coordinate frame), transforms the `ControlVectors` into world space, and generates a `Function<Double, Vector>` via `BezierUtil.cubicBezier3D()`.
...
- Add a new named curve: add an entry to `AttackType`.
