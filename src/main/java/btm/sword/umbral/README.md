...
| `LodgedState` | Blade fixed at impact location with glow. Registered as interactable so the wielder can dash-to-grab or use the grab ability. Starts entity follow task in `onEnter()` if `hitEntity` is set. | `DisplayUtil.itemDisplayFollow` task (stored in `followTask`) | `unregisterAsInteractableItem()`, `followTask.cancel()`, `resetFlightState()`, clears glow |
